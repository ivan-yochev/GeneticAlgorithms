...
# Кръстосване чрез смесване (Blend Crossover, BLX)

# При метода BLX всяко потомство се генерира случайно в интервал, определен от стойностите на двамата родители. Интервалът се
#  изчислява по формулата:

# [paren t1 − α(paren t2 − paren t1), paren t2 + α(paren t2 − paren t1)]

# Как влияе стойността на α:
# При α=0, интервалът съвпада със стойностите на родителите [p1, p2].
# С увеличаване на α, интервалът се разширява, което дава възможност за по-голяма вариация в гените на потомството.

# Пример:
# Ако стойностите на родителите са 1.33 и 5.72:
# α=0: интервалът е [1.33, 5.72].
# α=0.5: интервалът е [−0.865, 7.915] (два пъти по-широк).
# α=1.0: интервалът е [−3.06, 10.11] (три пъти по-широк).

# Обикновено α се задава със стойност 0.5, което предоставя баланс между разнообразие и стабилност в генетичната информация.


# Кръстосване чрез симулирано бинарно кръстосване (Simulated Binary Crossover, SBX)

# SBX имитира свойствата на едноточковото кръстосване, което обикновено се използва с бинарно кодирани хромозоми. Едно от тези свойства е,
#  че средната стойност на родителите е равна на средната стойност на потомците.

# Формула за SBX:
# Двете потомства се създават от родителите чрез следните формули:

# offspring1 = 1/2 * [(1 + β) * parent1 + (1 - β) * parent2]
# offspring2 = 1/2 * [(1 - β) * parent1 + (1 + β) * parent2]

# Където:
# β е случаен фактор, наречен коефициент на разпределение (spread factor).

# Свойства на SBX:
# Средна стойност: Средната стойност на двете потомства е равна на средната стойност на родителите, независимо от стойността на β.
# Когато β = 1: Потомците са идентични с родителите.
# Когато β < 1: Потомците са по-близо един до друг, отколкото родителите.
# Когато β > 1: Потомците са по-далеч един от друг, отколкото родителите.

# Пример:
# Ако стойностите на родителите са 1.33 и 5.72:
# При β = 0.8: Потомците ще бъдат 1.769 и 5.281.
# При β = 1.0: Потомците ще бъдат 1.33 и 5.72.
# При β = 1.2: Потомците ще бъдат 0.891 и 6.159.
# Средната стойност на потомците във всички случаи е 3.525, което е същото като средната стойност на родителите.

# Контрол на разпределението на β:
# За да се запази сходството между родителите и потомците, вероятността за стойности на β трябва да бъде най-висока около 1.
# Това се постига чрез изчисляване на β на база друга случайна стойност u, която е равномерно разпределена в интервала [0, 1].
# Ако u ≤ 0.5: β = (2u)^(1/(η+1))
# Иначе: β = [2(1 - u)]^(-1/(η+1))
# Където η е параметър, който контролира формата на разпределението. По-големи стойности на η правят β по-близо до 1.

# Заключение:
# SBX е ефективен метод за кръстосване, който запазва ключови свойства на бинарното кръстосване, като същевременно се адаптира
#  към реално кодирани хромозоми.

# Мутация при реално кодиране (Real Mutation)

# При генетични алгоритми с реално кодиране, един от начините за прилагане на мутация е замяната на която и да е реална стойност
#  с нова, генерирана случайно.

# Ограничения на този подход:
# Замяната със случайна стойност може да доведе до мутиран индивид, който няма връзка с оригиналния индивид.
# Алтернативен подход:
# Вместо да заменяме стойността напълно, се генерира случайно реално число, което се намира в близост до оригиналната стойност.
# Пример: Нормално разпределена (Gaussian) мутация:
# Генерира се случайно число, използвайки нормално разпределение със средна стойност 0 и предварително определено стандартно отклонение.
# Това позволява новата стойност да бъде по-близо до оригиналната, като по този начин запазва връзката между тях.
# Забележка:
# Следват два раздела, които ще разгледат напреднали теми: елитизъм и нишово разпределение (niching).

# Разбиране на елитизма (Elitism)

# Докато средната стойност на годността (fitness) в популацията на генетичния алгоритъм обикновено се увеличава с напредването на 
# поколенията, е възможно най-добрите индивиди от текущото поколение да бъдат загубени. Това се дължи на селекцията, кръстосването
#  и мутацията, които променят индивидите при създаването на следващото поколение.

# Възможна загуба: В много случаи тази загуба е временна, тъй като тези индивиди (или по-добри) могат да бъдат повторно въведени в 
# популацията в бъдещо поколение.
# Елитизъм:
# За да гарантираме, че най-добрите индивиди винаги преминават към следващото поколение, можем да приложим стратегията на елитизма:

#     Най-добрите n индивида (където n е малък, предварително зададен параметър) се дублират директно в следващото поколение.
    
#     Останалите места в популацията се запълват с потомци, създадени чрез селекция, кръстосване и мутация.
    
#     Елитните индивиди, които са дублирани, все още могат да участват в процеса на селекция, което означава, че могат да бъдат 
#     използвани като родители за нови индивиди.

# Предимства на елитизма:
# Елитизмът може значително да подобри производителността на алгоритъма, тъй като предотвратява загубата на добри решения, което би 
# довело до разхищаване на време за повторното им откриване.

# Нишово разпределение и споделяне (Niching and Sharing)

# В природата всяка среда е допълнително разделена на множество под-среди, или ниши, които са населени от различни видове,
#  използващи уникалните ресурси, достъпни във всяка ниша (като храна и подслон).

# Пример: В една гора има върхове на дървета, храсти, горски под, корени на дървета и т.н., всяко от които поддържа различни видове, 
# специализирани за живот в тази ниша и използващи нейните ресурси.
# Когато няколко различни вида съжителстват в една и съща ниша, те се конкурират за същите ресурси. Това води до тенденция за 
# търсене на нови, необитаеми ниши и тяхното заселване.
# Приложение в генетичните алгоритми:
# В контекста на генетичните алгоритми това нишово явление може да се използва за:

# Поддържане на разнообразие в популацията.
# Намиране на множество оптимални решения, всяко от които представлява отделна ниша.
# Пример:
# Да предположим, че нашият генетичен алгоритъм цели да максимизира функция за годност, която има няколко върха с различна височина.

# С напредването на алгоритъма повечето от популацията вероятно ще се концентрират около най-високия връх (глобалния максимум).
# Въпреки това, може да се наложи да намерим не само глобалния максимум, но и някои (или всички) от другите върхове.
# Решение:

# Да разглеждаме всеки връх като отделна ниша, която предлага ресурси, пропорционални на височината на върха.
# Да споделяме тези ресурси между индивидите, които обитават съответната ниша.
# Резултат:

# Най-високият връх привлича най-много индивиди, тъй като предлага най-голяма награда.
# Останалите върхове също се населяват от индивиди, но в намаляващи пропорции, съобразно тяхната "награда".
# Реализация на механизма за споделяне:
# Метод на разстоянията:
# Стойността на годността на всеки индивид се дели на (някаква функция на) комбинираните разстояния от всички останали индивиди.
# Метод на радиуса:
# Стойността на годността на всеки индивид се дели на броя на другите индивиди в рамките на определен радиус около него.
# Тези подходи подпомагат разпределянето на популацията в различни ниши и увеличават разнообразието в решенията. Ако желаете, мога 
# да продължа с повече подробности или примери!

# Сериен срещу паралелен подход за нишово разпределение (Serial Niching vs. Parallel Niching)

# Предизвикателства при нишовото разпределение:
# Както беше описано по-рано, прилагането на нишовото разпределение може да се окаже сложно, тъй като увеличава изчислителната 
# сложност на изчисляването на годността. Освен това изисква популацията да бъде увеличена до размер, равен на оригиналната популация,
#  умножена по броя на очакваните върхове (който обикновено е неизвестен).

# Сериен подход (Serial Niching):
# За да се преодолеят тези проблеми, вместо да се опитваме да намерим всички върхове едновременно (паралелен подход), можем да 
# търсим върховете един по един (сериен подход).

# Как работи серийният подход:

# Изпълнение на алгоритъма:
# Генетичният алгоритъм се изпълнява обичайно, за да се намери най-доброто решение (глобалния максимум или първия връх).
# Обновяване на функцията за годност:
# След намирането на върха, функцията за годност се актуализира така, че областта около открития максимум да се "изравни".
#  Това премахва върха от пространството на решенията.
# Повторение на процеса:
# Генетичният алгоритъм се изпълнява отново, за да намери следващия най-добър връх.
# Итерация:
# Процесът се повтаря, като при всяка итерация се намира следващият най-добър връх, докато не бъдат намерени всички върхове 
# или докато не се достигне някакъв критерий за спиране.
# Преимущества на серийния подход:
# По-малка сложност: Не изисква значително увеличаване на размера на популацията.
# По-ниски изчислителни разходи: Избягва сложността на изчисляване на разстоянията между индивидите във всяка итерация.
# Постепенно изследване: Подходящ за случаи, в които броят на върховете не е известен предварително.


# Изкуството на решаване на задачи с генетични алгоритми

# Генетичните алгоритми предоставят мощен и гъвкав инструмент, който може да бъде използван за решаване на широка гама от
#  проблеми и задачи. Когато работим върху нов проблем, е необходимо да адаптираме инструмента към специфичните изисквания на 
#  задачата. Това се постига чрез следните стъпки:

# 1. Определяне на функцията за годност (fitness function):
# Функцията оценява всеки индивид, като по-високите стойности означават по-добри индивиди.
# Не е задължително функцията да бъде математическа. Тя може да бъде:
# Алгоритъм.
# Обаждане към външна услуга.
# Резултат от игра, и др.
# Важно е само да имаме програмен начин за изчисляване на стойността на годността за всяко предложено решение.
# 2. Избор на подходящо кодиране на хромозомата (chromosome encoding):
# Това зависи от параметрите, които подаваме към функцията за годност.
# Примерни видове кодиране:
# Бинарно.
# Целочислено.
# Подреден списък.
# Реално кодиране.
# За някои проблеми може да се наложи смесване на типове параметри или създаване на персонализирано кодиране.
# 3. Избор на метод за селекция (selection method):
# Повечето методи за селекция работят с всякакъв тип хромозоми.
# Ако функцията за годност не е директно достъпна, но имаме начин да определим кое решение е по-добро сред няколко кандидати,
#  можем да използваме метода на турнирна селекция.
# 4. Избор на оператори за кръстосване и мутация (crossover and mutation operators):
# Тези избори зависят от кодирането на хромозомите:
# Бинарно кодирани хромозоми изискват различни схеми за кръстосване и мутация, в сравнение с тези за реално кодирани задачи.
# Както при кодирането на хромозомите, можете да създадете персонализирани методи за кръстосване и мутация, които са подходящи за 
# вашия конкретен случай.
# Генетичните алгоритми предоставят свобода и гъвкавост, които позволяват адаптиране към различни проблеми, като успехът зависи от
#  правилното настройване на всички тези компоненти.


# Хиперпараметри на алгоритъма

# Накрая, е необходимо да се зададат стойности за хиперпараметрите на генетичния алгоритъм. Най-често срещаните параметри,
#  които трябва да се настроят, са:

# Размер на популацията
# Вероятност за кръстосване (Crossover rate)
# Вероятност за мутация (Mutation rate)
# Максимален брой поколения (Max number of generations)
# Други условия за спиране (Other stopping condition(s))
# Елитизъм (Elitism): Дали да се използва, и ако да – какъв размер.
# За тези параметри можем да изберем стойности, които смятаме за разумни, и след това да ги настройваме, както се прави с 
# хиперпараметрите в почти всеки друг алгоритъм за оптимизация и машинно обучение.

# Важно:
# Ако изглежда сложно да направите всички тези избори наведнъж, не се притеснявайте! В следващите глави ще преминем през този 
# процес многократно за различни типове задачи.

# След като прочетете книгата, ще можете уверено да подхождате към нови проблеми и да правите разумни избори за настройка на 
# хиперпараметрите.