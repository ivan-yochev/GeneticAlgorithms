...
# ЕКСПЕРИМЕНТИРАНЕ С РАЗЛИЧНИ НАСТРОЙКИ. За всеки проблем няма универсално правило за избор на настройки. Вместо това трябва да
#  се изпробват различни комбинации и да се анализират резултатите, за да се намери най-добрата настройка за конкретния проблем.

# Размер на популацията и брой поколения

# Ще започнем експериментите, като направим промени в размера на популацията и броя поколения, използвани от генетичния алгоритъм:

# Размерът на популацията се определя от константата POPULATION_SIZE.
# Ще започнем, като увеличим стойността на тази константа от 200 на 400:

# POPULATION_SIZE = 400

# Ефект на промяната:

# Тази модификация ускорява генетичния поток.
# Най-доброто решение вече се намира след 22 поколения, както е показано на следващата фигура.

# След това ще опитаме да намалим размера на популацията на 100:

# POPULATION_SIZE = 100

# Ефект на промяната:

# Тази модификация забавя сближаването на алгоритъма, който вече няма да достигне най-добрата възможна стойност след 50 поколения.
# За компенсация ще увеличим стойността на MAX_GENERATIONS на 80:

# MAX_GENERATIONS = 80

# Това увеличава броя на поколенията, което дава на алгоритъма повече време да достигне оптимално решение.

# !!!!!!!
# Това поведение е типично за решения, базирани на генетични алгоритми – увеличаването на популацията ще изисква по-малко поколения,
# за да се достигне до решение.

# Въпреки това, изискванията за изчислителна мощ и памет се увеличават с размера на популацията.
# Обикновено се стремим да намерим умерен размер на популацията, който да осигури решение в рамките на разумно време.


# Оператор за кръстосване

# Нека нулираме промените и се върнем към първоначалните настройки (50 поколения, размер на популацията 200).
# Сега сме готови да експериментираме с оператора за кръстосване, който отговаря за създаването на потомци от родителски индивиди.

# Промяна на типа кръстосване:
# Смяната на типа кръстосване от едноточково (single-point) към двуточково (two-point) е проста:
# Дефинираме оператора mate, както следва:

# toolbox.register("mate", tools.cxTwoPoint)

# Резултат:
# След тази промяна алгоритъмът намира най-доброто решение само след 27 поколения.

# !!!!!
# Това поведение е типично за генетични алгоритми, които използват представяне чрез бинарни низове, тъй като двуточковото кръстосване
# предоставя по-гъвкав начин за комбиниране на два родителя и смесване на техните гени в сравнение с едноточковото кръстосване.

# Оператор за мутация

# Сега ще нулираме промените отново, за да експериментираме с оператора за мутация, който въвежда случайни модификации в потомците.

# 1. Увеличаване на P_MUTATION
# Започваме, като увеличим стойността на константата P_MUTATION на 0.9.
# Това води до следното поведение:
# Резултатите първоначално може да изглеждат изненадващи, тъй като увеличаването на честотата на мутация обикновено кара алгоритъма да се 
# държи непредсказуемо.
# Тук обаче ефектът е почти незабележим.
# Причина за това поведение:
# P_MUTATION:
# Определя вероятността даден индивид да бъде мутиран.
# indpb:
# Това е параметър, специфичен за оператора mutFlipBit, който задава вероятността всеки бит в индивида да бъде обърнат.
# тук се явавя втори параметър отговорен за мутацията.

# toolbox.register("mutate", tools.mutFlipBit, indpb=1.0/ONE_MAX_LENGTH)

# В програмата indpb е зададено като 1.0/ONE_MAX_LENGTH:
# При OneMax проблем с дължина 100 това означава, че средно само един бит ще бъде обърнат в мутирано решение.
# Това ограничава ефекта от мутацията, независимо от стойността на P_MUTATION.
# 2. Увеличаване на indpb десетократно
# Променяме стойността на indpb на:

# toolbox.register("mutate", tools.mutFlipBit, indpb=10.0/ONE_MAX_LENGTH)

# Резултат:
# Алгоритъмът започва да се държи непредсказуемо.
# Отначало подобрява решенията, но бързо попада в състояние на осцилации, без да прави значителен напредък.

# 3. Увеличаване на indpb още повече (до 50.0/ONE_MAX_LENGTH)
# Променяме стойността на indpb на:

# toolbox.register("mutate", tools.mutFlipBit, indpb=50.0/ONE_MAX_LENGTH)

# Резултат:
# Графиката изглежда нестабилна и показва, че генетичният алгоритъм се е превърнал в еквивалент на случаен търсене.
# Алгоритъмът не прави прогрес към по-добри решения, но може да открие най-доброто решение по случайност.
# Заключение:
# Малки стойности на indpb: Ограничават мутацията и правят алгоритъма стабилен.
# Големи стойности на indpb: Водят до хаотично поведение и превръщат алгоритъма в случайно търсене.
# Балансиране на P_MUTATION и indpb: Ключово за ефективността на генетичния алгоритъм.

# Оператор за селекция

# След това ще разгледаме оператора за селекция.

# Най-напред ще променим размера на турнира (tournament size), за да видим комбинирания ефект на този параметър с вероятността за мутация.
# След това ще разгледаме използването на рулетъчна селекция (roulette selection) вместо турнирна селекция.

# Размер на турнира и връзка с вероятността за мутация

# Отново ще се върнем към първоначалните настройки на програмата, преди да направим нови промени и да проведем някои експерименти:

# 1. Промяна на параметъра tournamentSize:
# Първо, ще променим параметъра tournsize на алгоритъма за турнирна селекция, като го зададем на 2 (вместо първоначалната стойност от 3):

# toolbox.register("select", tools.selTournament, tournsize=2)

# Резултат:
# Тази промяна не изглежда да има забележим ефект върху поведението на алгоритъма

# Какво ще стане, ако увеличим размера на турнира до много голяма стойност, например 100? Нека видим:

# toolbox.register("select", tools.selTournament, tournsize=100)

# Резултат:
# Алгоритъмът все още работи добре и намира най-доброто решение за по-малко от 40 поколения.
# Забележим ефект:
# Максималната годност (max fitness) сега силно прилича на средната годност (average fitness), както е показано на следващата графика.
# Това показва, че голям размер на турнира значително намалява разнообразието в популацията, което води до сходство между най-добрите
# и средните индивиди.

# Това поведение се наблюдава, защото когато размерът на турнира се увеличава, вероятността слабите индивиди да бъдат избрани намалява,
# а по-добрите решения започват да доминират в популацията.

# Ефекти върху реални проблеми:
# В реални проблеми това доминиране може да доведе до насищане на популацията с подоптимални решения, което предотвратява намирането на 
# най-доброто решение.
# Това явление е известно като преждевременно сближаване (premature convergence).
# Защо няма проблем при OneMax?
# В случая на простия проблем OneMax това не изглежда да е проблем.
# Възможно обяснение:
# Операторът за мутация осигурява достатъчно разнообразие, за да насочи решенията в правилната посока.
# Това подчертава значението на балансираното използване на селекция и мутация за поддържане на разнообразието в популацията.

# За да тестваме това обяснение, нека намалим вероятността за мутация десетократно до 0.01:

# P_MUTATION = 0.01

# Резултат от повторно изпълнение на алгоритъма:
# Поведение:

# Резултатите спират да се подобряват скоро след началото на алгоритъма.
# Последващите подобрения са много по-бавни, с рядка поява на подобрения тук и там.
# Общи резултати:

# Те са значително по-лоши в сравнение с предишното изпълнение.
# Най-добрата годност е около 80, вместо очакваните 100.

# Това тълкуване е, че поради големия размер на турнира най-добрите индивиди от началната популация вземат надмощие в рамките на малък 
# брой поколения, което се отразява в първоначалното бързо нарастване и на двете графики на плота.

# След това само случайна мутация в правилната посока – такава, която обръща 0 на 1 – създава по-добър индивид; това е показано на плота
# чрез скок на червената линия. Малко след това този индивид отново поема надмощие в цялата популация, като зелената линия догонва червената.

# За да направим тази ситуация още по-крайна, можем допълнително да намалим честотата на мутация:

# P_MUTATION = 0.001

# Сега можем да наблюдаваме същото общо поведение, но тъй като мутациите са много редки, подобренията са редки и отдалечени във времето.

# Ако увеличим броя на поколенията до 500, можем да видим това поведение по-ясно:

# MAX_GENERATIONS = 500

# От любопитство, нека върнем размера на турнира обратно на 3 и възстановим броя на поколенията на 50, като оставим малката 
# честота на мутация:
...
# Получената графика е много по-близка до оригиналната.
# Изглежда, че отново се е случило доминиране, но доста по-късно – около поколение 30, когато най-добрата годност вече е 
# близо до максималната стойност от 100.
# В този случай по-разумна честота на мутация би помогнала да се намери най-доброто решение, както се случи с оригиналните настройки.

# Рулетъчна селекция (Roulette Wheel Selection)

# Нека отново се върнем към оригиналните настройки, за да се подготвим за последния експеримент. Сега ще опитаме да заменим алгоритъма
# за турнирна селекция с рулетъчна селекция, която беше описана в Глава 2: Разбиране на ключовите компоненти на генетичните алгоритми.

# Промяната се извършва по следния начин:

# toolbox.register("select", tools.selRoulette)

# Тази промяна ще ни позволи да видим как се различава поведението на алгоритъма при използване на рулетъчна селекция вместо турнирна.

# Тази промяна изглежда влошава резултатите на алгоритъма.

# Както показва следващият плот:
# Съществуват множество моменти, в които най-доброто решение се губи в резултат на селекцията.
# Максималната годност (max fitness) намалява, поне временно, въпреки че средната годност (average fitness) продължава да се увеличава.
# Причина за това поведение:

# Алгоритъмът за рулетъчна селекция избира индивиди с вероятност, пропорционална на тяхната годност.
# Когато разликите между индивидите са относително малки:
# Има по-голям шанс по-слаби индивиди да бъдат избрани, в сравнение с турнирната селекция, която използвахме преди.
# Това показва, че изборът на подходящ метод за селекция зависи от спецификата на проблема и как се разпределя годността в популацията.

# За да компенсираме това поведение, можем да използваме елитистичния подход, споменат в Глава 2: Разбиране на ключовите компоненти
# на генетичните алгоритми.

# Елитистичният подход:

# Позволява определен брой от най-добрите индивиди в текущото поколение да преминат в следващото поколение без промяна.
# Това предотвратява загубата на тези индивиди.
# Предстоящи стъпки:

# В следващата глава ще разгледаме как да приложим елитистичния подход, използвайки библиотеката DEAP.

# Резюме

# В тази глава се запознахте с DEAP – гъвкава рамка за еволюционно изчисление, която ще бъде използвана в останалата част от книгата за 
# решаване на реални проблеми чрез генетични алгоритми.

# Научихте за модулите creator и toolbox в DEAP и как да ги използвате за създаване на различните компоненти, необходими за изпълнението
# на генетичния алгоритъм.

# С помощта на DEAP бяха написани две версии на Python програма за решаване на проблема OneMax:

# Първата включва пълна имплементация на генетичния алгоритъм.
# Втората е по-кратка и използва вградените алгоритми на рамката.
# Трета версия на програмата въведе функцията HOF (Hall of Fame), предоставена от DEAP.

# След това експериментирахме с различни настройки на генетичния алгоритъм и проучихме ефектите от промените в размера на популацията,
# както и в операторите за селекция, кръстосване и мутация.

# Какво следва:
# В следващата глава ще разширим наученото и ще започнем да решаваме реални комбинаторни проблеми, включително:

# Проблема на търговския пътник (Traveling Salesman Problem, TSP)
# Проблема с маршрутизирането на превозни средства (Vehicle Routing Problem, VRP)
# Това ще бъде постигнато чрез Python програми, базирани на DEAP.

# Допълнителна литература:

# DEAP документация: https://deap.readthedocs.io/en/master/
# Изходен код на DEAP в GitHub: https://github.com/DEAP/deap
